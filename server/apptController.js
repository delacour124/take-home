...
//   const params = [docId];
//   const query = 
//     'SELECT ptFirstName, ptLastName, date, time, kind FROM appointment WHERE doc_id = $1 ORDER BY time';
...
apptController.getAppointment = (req, res, next) => {
  ...
  const query = 
    'SELECT ptFirstName, ptLastName, date, time, kind FROM appointment WHERE doc_id = $1 AND date = $2 ORDER BY time';
  db.query(query, params)
    .then (data => {
      console.log(data.rows);
      res.locals.appointments = data.rows;
...
apptController.deleteAppointment = (req, res, next) => {
  ...
  const query = 'DELETE FROM appointment WHERE doc_id = $1 AND appt_id = $2'
  db.query(query, params)
    .then(data => {
      return next();
    })
...
apptController.addAppointment = (req, res, next) => {
  ...
  const query = "INSERT INTO appointments (doc_id, ptFirstName, ptLastName, date, time, kind) VALUES ($1, $2, $3, $4, $5, $6) WHERE (SELECT COUNT(*) FROM appointment WHERE time = $5) <= 3 CHECK (MINUTE(time) = '00' or MINUTE(time) = '15' or MINUTE(time) = '30' or MINUTE(time) = '45')";
